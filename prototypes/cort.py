...
class EnhancedRecursiveThinkingChat:
    def __init__(
        self, api_key: str = None, model: str = "o4-mini", use_copilot: bool = False
    ):
        """Initialize with OpenAI API or GitHub Copilot API."""
...
def main():
    ...
    # Check if GitHub Copilot API key exists
    ...
    if copilot_api_key:
        ...
        use_copilot = True
        print("Using GitHub Copilot API...")
    else:
        # Get API key
        api_key = input(
            "Enter your OpenAI API key (or press Enter to use env variable): "
        ).strip()
        if not api_key:
            api_key = os.getenv("OPENAI_API_KEY")
            if not api_key:
                print(
                    "Error: No API key provided and OPENAI_API_KEY not found in environment"
                )
                return

    # Initialize chat
    chat = EnhancedRecursiveThinkingChat(api_key=api_key, use_copilot=use_copilot)
